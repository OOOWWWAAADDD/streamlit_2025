...
if st.session_state.forgot_password:
    with st.expander("パスワードお問い合わせ"):
        ...
        if check:
            ...
            img_pass = None  # チェックしてなければ値なし
        
        # img_passが入力されているかつロボットチェックOKならボタンを表示
      ...
      #  mail_a = st.text_input("メールアドレスを入力してください")
      #  aa_button = st.button("送信する")
       # if aa_button:
          #  if mail_a == "s1313@cst.otsuma.ac.jp":
        #        print(st.session_state.mail)
   #             st.write(st.session_state.mail)
    #            check = st.checkbox('私はロボットではありません')
    #            if check:
...
with st.sidebar.expander("お問い合わせフォーム"):
    with st.sidebar.expander("パスワードお問い合わせ"):
        mail = st.text_input("メールアドレスを入力してください")
        passbutton = st.button("送信する")

if mail and passbutton:
    if mail == "s1313@cst.otsuma.ac.jp":
        selected_item = st.sidebar.radio(
                  'お問い合わせ内容を選択してください。',
                  ['成績開示', 'パスワード請求','その他']
                )
        a_button = st.sidebar.button('送信')

        if selected_item  == '成績開示' and a_button:
            st.sidebar.info("提示可能な成績がありません")
        elif selected_item == 'パスワード請求'and a_button:
            st.sidebar.info("パスワードはWebProgrammingです")
        elif selected_item == 'その他' and a_button:
